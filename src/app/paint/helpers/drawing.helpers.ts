...
import { COLOR_WHITE } from '../services/paint.config';
...
export function drawLine(
  ...
  bresenhamLinePlot([x0, y0, x1, y1], (x, y) =>
    setPixelInOriginalImage({ w: x, h: y }, color, image)
  );
}
...
export function setPixelInOriginalImage(
  point: Point,
  color: Color,
  image: ImageData
) {
...
export function getPixel(point: Point, image: ImageData): Color {
  ...
  if (pixelOffset !== undefined) {
    color = {
      r: image.data[pixelOffset],
      g: image.data[pixelOffset + 1],
      b: image.data[pixelOffset + 2],
    };
  }
...
function horizontalBresenhamLinePlot(
  [x0, y0, x1, y1]: number[],
  plot: (x: any, y: any) => void
) {

  ...
  implements OnInit, OnDestroy, AfterViewInit
{
  ...
  ngOnInit(): void {
    this._viewportScrolled
      .pipe(debounceTime(400), takeUntil(this._ngUnsubscribe))
      .subscribe((scrollPosition) => {
        this.scrollPositionChange.emit(scrollPosition);
      });
    this._viewportResized
      .pipe(debounceTime(400), takeUntil(this._ngUnsubscribe))
    ...
    if (this._isModelLocked) {
      ...
      if (event.srcElement) {
        ...
        const scrollPosition: Point = {
          w: event?.srcElement?.scrollLeft,
          h: event?.srcElement?.scrollTop,
        };
